...
function App() {
  ...
  const claimNFTs = (cost) => {
    const selectedMintAmount = cost > 0 ? mintAmount : freeMintAmount
    let gasLimit = CONFIG.GAS_LIMIT
    let totalCostWei = String(cost * selectedMintAmount)
    let totalGasLimit = String(gasLimit * selectedMintAmount)
    setFeedback(`Minting your ${CONFIG.NFT_NAME}...`)
    setClaimingNft(true)
    blockchain.smartContract.methods
      .mint(selectedMintAmount)
      .send({
      ...
      .then((receipt) => {
        console.log(receipt)
        if (cost === 0) {
          localStorage.setItem("freeMinted", 1)
        }
        setFeedback(
            ...
            {Number(data.totalSupply) >= CONFIG.MAX_SUPPLY ? (
              ...
              <>
                ...
                >
                  ...
                  AUUUGUGGGHHH IF U MINT THIS YU AARE A KLOOOWWNN
                  <br />
                  <br />
                  DA FIRST 1000 KLOWNS R FREEE, DA REST ARE .003Ξ
                  <br />
